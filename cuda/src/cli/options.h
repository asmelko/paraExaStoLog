...
public:
	parsed_param_base(std::string name, bool parse_ok, bool constr_ok)
		: name_(std::move(name)), p_ok_(parse_ok), constr_ok_(constr_ok)
	{}
...
public:
	parsed_param(T parsed_value, std::string name, bool parse_ok, bool constr_ok)
		: parsed_param_base(std::move(name), parse_ok, constr_ok), parsed_value(std::move(parsed_value))
	{}
...
{
public:
	parsed_option(std::string name, std::vector<std::unique_ptr<parsed_param_base>> params, bool parse_ok,
				  bool constr_ok, bool count_ok, bool unknown)
		: name_(std::move(name)),
...
public:
	parsed_args(std::unordered_map<std::string, parsed_option*> options_mapping, std::deque<parsed_option> options,
				std::vector<std::string> plain_arguments, bool parse_ok)
...
protected:
	parameter_base(const std::string name, bool mandatory) : name(name), mandatory(mandatory) {}
...
public:
	...
	parameter(const std::string name, bool mandatory = true) : parameter_base(name, mandatory), constraint_(nullptr) {}
	parameter(const std::string name, bool mandatory, constr_t constraint)
		: parameter_base(name, mandatory), constraint_(constraint)
	{}
...
public:
	...
	option(std::string names, const std::string description, bool optional = true)
		: names(construct_names_(std::move(names))), description(description), optional(optional)
	{}
...
class command
{
	...
	{
	public:
		option_adder(command& owner) : owner_(owner) {}

		// adds option with a parameter and its contraint
		template <typename T>
		option_adder& operator()(std::string names, const std::string description, value_type<T>,
								 std::string param_name, typename ::mbas::parameter<T>::constr_t param_constraint)
		{
			owner_.add_option<T>(std::move(names), std::move(description), true,
								 { parameter<T>(param_name, true, param_constraint) });
			return *this;
		}
		...
		{
			owner_.add_option<T>(std::move(names), std::move(description), true,
								 { parameter<T>(std::move(param_name)) });
			return *this;
		}
		...
		template <typename T>
		option_adder& operator()(std::string names, const std::string description, value_type<T>,
								 std::string param_name, bool optional)
		{
			owner_.add_option<T>(std::move(names), std::move(description), optional,
								 { parameter<T>(std::move(param_name)) });
			return *this;
		}
...
public:
	...
	template <typename... T>
	option& add_option(std::string names, std::string description, bool optional,
					   typename ::mbas::parameter_holder<T...>::type params)
	{
		options_.emplace_back(std::move(names), std::move(description), optional);
...
private:
	...
	public:
		parser(const command* cmd, int argc, char** argv) : cmd(cmd), argc(argc), argv(argv) {}
		...
		void parse_option(size_t& i)
		{
			...
			if (first_par_parsed)
			{
				...
				add_parameter(*opt->parameters()[0], std::move(first_par), option_parse_ok, option_constraint_ok,
							  opt_params);
				parsed_par_count = 1;
			}
			else if (!opt)
			{
				options.emplace_back(option_name, std::vector<std::unique_ptr<parsed_param_base>> {}, false, true, true,
									 true);
				options_mapping.emplace(option_name, &options.back());
				return;
			}
			...
			while (parsed_par_count < opt->parameters().size())
			{
				...
				{
					add_parameter(*opt->parameters()[parsed_par_count], argv[i], option_parse_ok, option_constraint_ok,
								  opt_params);
					++parsed_par_count;
				}
			}
			...
			options.emplace_back(option_name, std::move(opt_params), option_parse_ok, option_constraint_ok, count_ok,
								 false);
			if (!option_parse_ok)
				parse_ok = false;
		...
		bool parse_short_options(char* arg, const option*& opt, std::string& option_name, std::string& first_par)
		{
			...
			while (opt && opt->parameters().size() == 0 && arg[i] != '\0')
			{
				...
				if (opt)
				{
					options.emplace_back(prev_name, std::vector<std::unique_ptr<parsed_param_base>> {}, true, true,
										 true, false);
		...
		void add_parameter(const parameter_base& parameter, std::string value, bool& p_ok, bool& constraint_ok,
...
std::unique_ptr<parsed_param_base> parameter_base_accessor::parse(const parameter_base& parameter,
