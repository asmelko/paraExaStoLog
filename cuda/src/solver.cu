...
void solver::solve_terminal_part()
{
	...
	for (size_t terminal_scc_idx = 1; terminal_scc_idx < terminals_offsets_.size(); terminal_scc_idx++)
	{
		...
		thrust::host_vector<index_t> h_scc_indptr_tmp = scc_indptr;
		thrust::host_vector<index_t> h_scc_rows_tmp = scc_rows;
		...
		for (int i = 0; i < h_scc_indptr_tmp.size() - 1; i++)
		{
			auto begin = h_scc_indptr_tmp[i];
			auto end = h_scc_indptr_tmp[i + 1];
