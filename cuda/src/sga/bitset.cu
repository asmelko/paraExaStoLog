...
__global__ void fwd_step(int m, const int* __restrict__ row_offsets, const int* __restrict__ column_indices,
						 unsigned* __restrict__ colors, unsigned char* __restrict__ status, int* __restrict__ scc_root,
...
__global__ void fwd_step_lb(int m, const int* __restrict__ row_offsets, const int* __restrict__ column_indices,
							unsigned char* __restrict__ status, int* __restrict__ scc_root, bool* __restrict__ changed)
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void bwd_step(int m, const int* __restrict__ row_offsets, const int* __restrict__ column_indices,
						 unsigned* __restrict__ colors, unsigned char* __restrict__ status, bool* __restrict__ changed)
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	int src = tid;
	// if src is in the forward frontier (valid, visited but not expanded)
...
__global__ void bwd_step_lb(int m, const int* __restrict__ row_offsets, const int* __restrict__ column_indices,
							unsigned char* __restrict__ status, bool* __restrict__ changed)
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	typedef cub::BlockScan<int, BLOCK_SIZE> BlockScan;
	const int SCRATCHSIZE = BLOCK_SIZE;
...
__global__ void trim_kernel(int m, const int* __restrict__ in_row_offsets, const int* __restrict__ in_column_indices,
							const int* __restrict__ out_row_offsets, const int* __restrict__ out_column_indices,
							unsigned* __restrict__ colors, unsigned char* __restrict__ status,
							int* __restrict__ scc_root, bool* __restrict__ changed)
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	int src = tid;
	if (src < m && !is_removed(status[src]))
	{
		...
		for (int offset = row_begin; offset < row_end; ++offset)
		{
			...
			if (!is_removed(status[dst]) && colors[dst] == colors[src])
			{
				in_degree++;
				break;
			}
		...
		if (in_degree == 0 || out_degree == 0)
		{
			...
			scc_root[src] = src;
			if (debug)
				printf("found vertex %d trimmed\n", src);
			*changed = true;
		}
	}
}

__global__ void trim2_kernel(int m, const int* __restrict__ in_row_offsets, const int* __restrict__ in_column_indices,
							 const int* __restrict__ out_row_offsets, const int* __restrict__ out_column_indices,
							 unsigned* __restrict__ colors, unsigned char* __restrict__ status,
							 int* __restrict__ scc_root)
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	int src = tid;
	if (src < m && !is_removed(status[src]))
...
__global__ void first_trim_kernel(int m, const int* __restrict__ in_row_offsets,
								  const int* __restrict__ in_column_indices, const int* __restrict__ out_row_offsets,
								  const int* __restrict__ out_column_indices, unsigned char* __restrict__ status,
								  bool* __restrict__ changed)
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	int src = tid;
...
__global__ void update_kernel(int m, unsigned* colors, unsigned char* status, unsigned* locks, int* scc_root,
							  bool* has_pivot)
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	int src = tid;
	if (src < m && !is_removed(status[src]))
	{
		unsigned new_subgraph =
			(is_fwd_visited(status[src]) ? 1 : 0)
			+ (is_bwd_visited(status[src]) ? 2 : 0); // F intersec B == 3, F/B == 1 B/F == 2 (V/F)/B == 0
		if (new_subgraph == 3)
		{
			set_removed(&status[src]);
...
void fwd_reach(int m, const int* out_row_offsets, const int* out_column_indices, unsigned* colors,
			   unsigned char* status, int* scc_root)
{
...
void fwd_reach_lb(int m, const int* out_row_offsets, const int* out_column_indices, unsigned char* status,
				  int* scc_root)
{
	int nthreads = BLOCK_SIZE;
	int nblocks = (m - 1) / nthreads + 1;
...
void iterative_trim(int m, const int* in_row_offsets, const int* in_column_indices, const int* out_row_offsets,
					const int* out_column_indices, unsigned* colors, unsigned char* status, int* scc_root)
{
	int nthreads = BLOCK_SIZE;
	int nblocks = (m - 1) / nthreads + 1;
	bool h_changed, *d_changed;
	...
	do
	{
		...
		h_changed = false;
		CUDA_SAFE_CALL(cudaMemcpy(d_changed, &h_changed, sizeof(bool), cudaMemcpyHostToDevice));
		trim_kernel<<<nblocks, nthreads>>>(m, in_row_offsets, in_column_indices, out_row_offsets, out_column_indices,
										   colors, status, scc_root, d_changed);
...
void first_trim(int m, const int* in_row_offsets, const int* in_column_indices, const int* out_row_offsets,
				const int* out_column_indices, unsigned char* status)
{
	int nthreads = BLOCK_SIZE;
	int nblocks = (m - 1) / nthreads + 1;
	bool h_changed, *d_changed;
	...
	do
	{
		...
		h_changed = false;
		CUDA_SAFE_CALL(cudaMemcpy(d_changed, &h_changed, sizeof(bool), cudaMemcpyHostToDevice));
		first_trim_kernel<<<nblocks, nthreads>>>(m, in_row_offsets, in_column_indices, out_row_offsets,
												 out_column_indices, status, d_changed);
		CudaTest("solving kernel trim failed");
		CUDA_SAFE_CALL(cudaMemcpy(&h_changed, d_changed, sizeof(bool), cudaMemcpyDeviceToHost));
	} while (h_changed);
	CUDA_SAFE_CALL(cudaFree(d_changed));
...
void trim2(int m, const int* in_row_offsets, const int* in_column_indices, const int* out_row_offsets,
		   const int* out_column_indices, unsigned* colors, unsigned char* status, int* scc_root)
{
	int nthreads = BLOCK_SIZE;
	int nblocks = (m - 1) / nthreads + 1;
	trim2_kernel<<<nblocks, nthreads>>>(m, in_row_offsets, in_column_indices, out_row_offsets, out_column_indices,
										colors, status, scc_root);
...
void print_statistics(int m, int* scc_root, unsigned char* status)
{
	...
	for (int i = 0; i < m; i++)
	{
		if (scc_sets[i].size() == 1)
			num_trivial_scc++;
		else if (scc_sets[i].size() > 1)
			num_nontrivial_scc++;
		if (scc_sets[i].size() > biggest_scc_size)
			biggest_scc_size = scc_sets[i].size();
	}
	printf("\tnum_trimmed=%d, num_pivots=%d, total_num_scc=%d\n", total_num_trimmed, total_num_pivots,
		   total_num_trimmed + total_num_pivots);
	printf("\tnum_trivial=%d, num_nontrivial=%d, total_num_scc=%d, biggest_scc_size=%d\n", num_trivial_scc,
		   num_nontrivial_scc, total_num_scc, biggest_scc_size);
}
